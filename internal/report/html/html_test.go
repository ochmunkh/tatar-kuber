...
func TestRender_HTML(t *testing.T) {
	...
	s := b.String()
	for _, want := range []string{"TATAR-Kuber Security Report", "78/100", "TATAR-CON-001", "Privileged container", "production", "securityContext.privileged=false", "Засвар (Fix)", "Evidence (нотолгоо)", "spec.template.spec.containers[0].securityContext.privileged"} {
